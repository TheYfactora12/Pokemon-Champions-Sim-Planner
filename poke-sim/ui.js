...
// imported/custom teams appear in BOTH sides, with custom teams ordered first.
// ============================================================
var currentPlayerKey = 'player';

function isCustomTeamRecord(team) {
  return !!(team && team.source === 'custom');
}

function getOrderedTeamKeys() {
  ...
  for (var key in TEAMS) {
    ...
    if (isCustomTeamRecord(team)) custom.push(key);
    else preloaded.push(key);
  }
  return custom.concat(preloaded);
}

function getOrderedOpponentTeamKeys(playerKey, opts) {
  opts = opts || {};
  var includePlayer = !!opts.includePlayer;
  var ladderOnly = !!opts.ladderOnly;
  return getOrderedTeamKeys().filter(function(key) {
    if (!includePlayer && key === playerKey) return false;
    if (ladderOnly && typeof isLadderLegal === 'function' && !isLadderLegal(key)) return false;
    return true;
  });
}
...
function rebuildTeamSelects() {
  ...
  var playerKeys = getOrderedTeamKeys();
  var opponentKeys = getOrderedOpponentTeamKeys(prevPlayer, { includePlayer: false, ladderOnly: false });
  playerSel.innerHTML = '';
  if (Array.isArray(playerSel.options)) playerSel.options.length = 0;
  oppSel.innerHTML = '';
  if (Array.isArray(oppSel.options)) oppSel.options.length = 0;
  ...
  for (var j = 0; j < opponentKeys.length; j++) {
    var oppKey = opponentKeys[j];
    var oppTeam = TEAMS[oppKey];
    if (!oppTeam || !oppTeam.name) continue;
    var o2 = document.createElement('option');
    o2.value = oppKey; o2.textContent = oppTeam.name;
    oppSel.appendChild(o2);
  }
  if (TEAMS[prevPlayer]) playerSel.value = prevPlayer;
  if (opponentKeys.indexOf(prevOpp) >= 0) oppSel.value = prevOpp;
  else if (opponentKeys.length) oppSel.value = opponentKeys[0];
...
document.getElementById('player-select').addEventListener('change', function() {
  ...
  if (team) {
    ...
    var oppSel = document.getElementById('opponent-select');
    if (oppSel && TEAMS[oppSel.value]) {
      document.getElementById('opp-team-name').textContent = TEAMS[oppSel.value].name;
      renderRoster('opp-roster', TEAMS[oppSel.value].members);
    }
    ...
    if (typeof csStartFreshSimResultScope === 'function') csStartFreshSimResultScope(this.value, (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles'));
...
function _firstDifferentTeamKey(excludeKey) {
  var keys = getOrderedOpponentTeamKeys(excludeKey, {
    includePlayer: false,
    ladderOnly: !!LADDER_MODE
  });
...
function displayResults(res, oppKey) {
  ...
  try { csStoreTeamRunSnapshot(currentPlayerKey, oppKey, currentFormat, currentBo, 'single_sim_result'); } catch (_snapshotErr) {}
  revealPdfButton();
}
  ...
  host.innerHTML =
    ...
    (simComparison ? '<div class="replay-coach-card">' +
      ...
      '<div class="replay-coach-summary-grid">' +
        ...
        '<div class="replay-coach-metric"><strong>Team match</strong><span>' + _escapeHtml(replayTeamMatch ? (replayTeamMatch.status || 'unknown') : 'unknown') + '</span></div>' +
      ...
      '</div>' +
      '<div class="replay-coach-list">' +
        (replayTeamMatch ? '<div class="replay-coach-list-row"><strong>Replay team match</strong>' + _escapeHtml(replayTeamMatch.summary || '') + '<small>' + _escapeHtml((replayTeamMatch.evidence || []).join(' · ')) + '</small><small>' + _escapeHtml(replayTeamMatch.recommendedNextStep || '') + '</small></div>' : '') +
...
function csNormalizeReplayNameList(list) {
  return (list || []).map(csNormalizeReplayName).filter(Boolean).filter(function(name, idx, arr) {
    return arr.indexOf(name) === idx;
  });
}

function csSimpleReplayFingerprint(list) {
  return csNormalizeReplayNameList(list).sort().join('|');
}

function csReplayRulesetBucket(value) {
  value = String(value || '').toLowerCase();
  if (!value) return 'unknown';
  if (/champion/.test(value)) return 'champion';
  if (/generic|showdown|nationaldex/.test(value)) return 'generic';
  if (/single/.test(value)) return 'singles';
  if (/double|vgc/.test(value)) return 'doubles';
  return value;
}

function csBuildActiveTeamFingerprint(teamKey, fmt) {
  var key = teamKey || ((typeof currentPlayerKey === 'string' && currentPlayerKey) ? currentPlayerKey : 'player');
  var team = (typeof TEAMS !== 'undefined' && TEAMS[key]) ? TEAMS[key] : null;
  if (!team || !Array.isArray(team.members)) return null;
  var format = fmt || (typeof currentFormat !== 'undefined' ? currentFormat : (team.format || 'doubles'));
  var rulesetProfile = null;
  try {
    if (typeof csInferRulesetProfile === 'function') rulesetProfile = csInferRulesetProfile(format, team);
  } catch (_e) {
    rulesetProfile = null;
  }
  var species = team.members.map(function(member) { return member && member.name; }).filter(Boolean);
  return {
    teamId: key,
    teamProfileId: key,
    teamVersionId: key + ':' + format,
    displayName: team.name || key,
    format: format,
    ruleset: (rulesetProfile && (rulesetProfile.compatibilityClass || rulesetProfile.formatTag || rulesetProfile.label)) || 'champion_sim',
    speciesSet: csNormalizeReplayNameList(species),
    selectedPokemon: [],
    source: team.source || 'bundled',
    confidence: 'high',
    fingerprint: csSimpleReplayFingerprint(species)
  };
}

function csBuildReplayTeamFingerprint(parsed, selectedSide) {
  parsed = parsed || {};
  var side = selectedSide || parsed.selectedSide || 'p1';
  ...
  var species = preview.length ? preview : selected;
  var rulesetProfile = parsed.rulesetProfile || {};
  var format = parsed.formatKind || parsed.gametype || parsed.format || 'unknown';
  var confidence = preview.length >= 6 ? 'high' : (preview.length >= 4 ? 'medium' : (selected.length >= 3 ? 'medium' : 'low'));
  return {
    teamId: parsed.battleId || parsed.id || 'replay',
    teamProfileId: '',
    teamVersionId: '',
    displayName: parsed.playerNames && parsed.playerNames[side] ? parsed.playerNames[side] : 'Replay team',
    format: format,
    ruleset: rulesetProfile.compatibilityClass || rulesetProfile.formatTag || format,
    speciesSet: csNormalizeReplayNameList(species),
    selectedPokemon: csNormalizeReplayNameList(selected),
...
function csCompareTeamFingerprints(snapshot, replayFingerprint) {
  if (!snapshot || !snapshot.teamFingerprint || !replayFingerprint) return null;
  var simTeam = snapshot.teamFingerprint;
  var replayTeam = replayFingerprint;
  var simSpecies = csNormalizeReplayNameList(simTeam.speciesSet || []);
  var replaySpecies = csNormalizeReplayNameList(replayTeam.speciesSet || []);
  var overlapNames = replaySpecies.filter(function(name) { return simSpecies.indexOf(name) >= 0; });
  var overlapCount = overlapNames.length;
  var scoreBase = Math.max(simSpecies.length, replaySpecies.length, 1);
  var formatMatch = !!(simTeam.format && replayTeam.format && simTeam.format === replayTeam.format);
  var rulesetMatch = csReplayRulesetBucket(simTeam.ruleset) === csReplayRulesetBucket(replayTeam.ruleset)
    || csReplayRulesetBucket(simTeam.ruleset) === 'unknown'
    || csReplayRulesetBucket(replayTeam.ruleset) === 'unknown';
  var status = 'unknown';
  if (formatMatch && rulesetMatch && simSpecies.length >= 6 && replaySpecies.length >= 6 && overlapCount >= 6) status = 'same_team';
  else if (formatMatch && rulesetMatch && overlapCount >= 5) status = 'similar_team';
  else if (formatMatch && overlapCount >= 3) status = 'possible_match';
  else if (formatMatch && replaySpecies.length) status = 'different_team';
  var allowsSimComparison = status === 'same_team' || status === 'similar_team';
  var summary = 'Unknown team match.';
  if (status === 'same_team') summary = 'Replay looks like the same team as the current simulated team.';
  else if (status === 'similar_team') summary = 'Replay looks like a similar version of the current simulated team.';
  else if (status === 'possible_match') summary = 'Replay only partially matches the current simulated team.';
  else if (status === 'different_team') summary = 'Replay does not look like the current simulated team.';
  if (!formatMatch) summary = 'Replay format does not match the current simulation format.';
  else if (!rulesetMatch) summary = 'Replay ruleset does not match the current simulation ruleset.';
  return {
    status: status,
    similarityScore: Math.round((overlapCount / scoreBase) * 100),
    speciesOverlap: overlapCount,
    overlapNames: overlapNames,
    formatMatch: formatMatch,
    rulesetMatch: rulesetMatch,
    confidence: replayFingerprint.confidence || 'low',
    blockers: [
      formatMatch ? null : 'format_mismatch',
      rulesetMatch ? null : 'ruleset_mismatch',
      replaySpecies.length ? null : 'missing_replay_team_data'
    ].filter(Boolean),
    allowsSimComparison: allowsSimComparison,
    evidence: [
      overlapCount + '/' + scoreBase + ' species overlap',
      'format ' + (formatMatch ? 'matched' : 'mismatched'),
      'ruleset ' + (rulesetMatch ? 'matched' : 'mismatched')
    ],
    summary: summary,
    recommendedNextStep: allowsSimComparison
      ? 'Sim comparison can use this session team snapshot.'
      : (status === 'possible_match'
        ? 'Run a sim with this exact team version before trusting sim-vs-replay deltas.'
        : 'Run a sim with this team first, or switch back to the team that produced the current session snapshot.')
  };
}

function csStoreTeamRunSnapshot(teamKey, oppKey, fmt, bo, source) {
  if (typeof ChampionsSim === 'undefined') return null;
  var activeFingerprint = csBuildActiveTeamFingerprint(teamKey, fmt);
  if (!activeFingerprint) return null;
  var oppFingerprint = (typeof TEAMS !== 'undefined' && TEAMS[oppKey]) ? csBuildActiveTeamFingerprint(oppKey, fmt) : null;
  var now = new Date().toISOString();
  var snapshot = {
    id: 'team-run:' + (teamKey || 'player') + ':' + (fmt || activeFingerprint.format) + ':' + now,
    teamFingerprint: activeFingerprint,
    opponentFingerprint: oppFingerprint,
    opponentKey: oppKey || '',
    opponentName: oppKey && TEAMS[oppKey] && TEAMS[oppKey].name ? TEAMS[oppKey].name : '',
    format: fmt || activeFingerprint.format,
    ruleset: activeFingerprint.ruleset,
    bo: bo || null,
    createdAt: now,
    expiresAt: now,
...
function csBuildReplayTeamMatch(parsed, selectedSide) {
  if (typeof ChampionsSim === 'undefined') return null;
  var snapshot = ChampionsSim.state.lastTeamRunSnapshot || null;
  var replayFingerprint = csBuildReplayTeamFingerprint(parsed, selectedSide);
  if (!snapshot || !snapshot.teamFingerprint) {
    return {
      status: 'unknown',
      similarityScore: 0,
      speciesOverlap: 0,
      ...
      formatMatch: false,
      rulesetMatch: false,
      confidence: replayFingerprint && replayFingerprint.confidence ? replayFingerprint.confidence : 'low',
      blockers: ['missing_session_snapshot'],
      allowsSimComparison: false,
      evidence: ['No session team snapshot is available yet.'],
      summary: 'No session sim snapshot exists for team matching yet.',
      recommendedNextStep: 'Run a sim with this team first, then upload the replay in the same browser session.',
      replayFingerprint: replayFingerprint,
      teamFingerprint: null
    };
  }
  var match = csCompareTeamFingerprints(snapshot, replayFingerprint) || {};
  match.replayFingerprint = replayFingerprint;
  match.teamFingerprint = snapshot.teamFingerprint;
  match.snapshot = snapshot;
  return match;
}

function csBuildBattleSenseiSimPlan(parsed, selectedSide, replayTeamMatch) {
  parsed = parsed || {};
  ...
  if (replayTeamMatch && replayTeamMatch.allowsSimComparison === false) return null;
  ...
  return {
    ...
    playerPreview: (replayTeamMatch && replayTeamMatch.teamFingerprint && replayTeamMatch.teamFingerprint.speciesSet) ? replayTeamMatch.teamFingerprint.speciesSet : [],
    playerFingerprint: (replayTeamMatch && replayTeamMatch.teamFingerprint && replayTeamMatch.teamFingerprint.fingerprint) || '',
    replayTeamMatch: replayTeamMatch || null,
  ...
  runBtn.addEventListener('click', function() {
    ...
    try {
      ...
      if (typeof api.parseShowdownLog === 'function' && typeof api.buildReplayCoachReview === 'function') {
        ...
        opts.replayTeamMatch = csBuildReplayTeamMatch(parsed, selectedSide);
        opts.simPlan = csBuildBattleSenseiSimPlan(parsed, selectedSide, opts.replayTeamMatch);
        opts.sampleSize = opts.simPlan && opts.simPlan.sampleSize ? opts.simPlan.sampleSize : 1;
        var analysis = { parsed: parsed, review: api.buildReplayCoachReview(parsed, opts) };
      } else {
...
function _chooseCombos(source, count) {
  var out = [];
  var arr = Array.isArray(source) ? source.slice() : [];
  if (!count || count < 1 || count > arr.length) return out;
  function walk(start, picked) {
    if (picked.length === count) {
      out.push(picked.slice());
      return;
    }
    for (var i = start; i <= arr.length - (count - picked.length); i++) {
      picked.push(arr[i]);
      walk(i + 1, picked);
      picked.pop();
    }
...
function buildFormatAwareTeamPlans(teamKey, format) {
  var team = TEAMS[teamKey];
  if (!team || !Array.isArray(team.members) || !team.members.length) return [];
  var fmt = (format === 'singles') ? 'singles' : 'doubles';
  var bringCount = (fmt === 'singles') ? 3 : 4;
  var leadCount = (fmt === 'singles') ? 1 : 2;
  var names = team.members.map(function(m) { return m && m.name; }).filter(Boolean);
  if (names.length < bringCount) return [];
  var plans = [];
  var bringCombos = _chooseCombos(names, bringCount);
  for (var i = 0; i < bringCombos.length; i++) {
    var bring = bringCombos[i];
    var leadCombos = _chooseCombos(bring, leadCount);
    for (var j = 0; j < leadCombos.length; j++) {
      var lead = leadCombos[j];
      var bench = bring.filter(function(name) { return lead.indexOf(name) < 0; });
      plans.push({
        lead: lead.slice(),
        bring: lead.concat(bench)
      });
    }
  }
  return plans;
...
ChampionsSim.bring.buildFormatAwareTeamPlans = buildFormatAwareTeamPlans;
...
if (typeof exposeLegacyWindowAlias === 'function') exposeLegacyWindowAlias('buildFormatAwareTeamPlans', buildFormatAwareTeamPlans);
...
async function runBoSeries(numSeries, playerTeamKey, oppTeamKey, bo, onProgress, opts) {
  opts = opts || {};
  ...
  const playerPlans = Array.isArray(opts.playerPlans) ? opts.playerPlans.slice() : [];
  const effectiveSeriesCount = playerPlans.length ? Math.max(numSeries, playerPlans.length) : numSeries;
  results.playerPlanCatalogSize = playerPlans.length;
  results.seriesCountRequested = numSeries;
  results.seriesCountExecuted = effectiveSeriesCount;
  results.planMode = playerPlans.length ? 'exhaustive_player' : 'standard';

  for (let i=0; i<effectiveSeriesCount; i+=BATCH) {
    const bSize = Math.min(BATCH, effectiveSeriesCount-i);
    for (let j=0; j<bSize; j++) {
      ...
      const playerPlan = playerPlans.length ? playerPlans[(i + j) % playerPlans.length] : null;
      const playerBring   = (playerPlan && Array.isArray(playerPlan.bring) && playerPlan.bring.length)
        ? playerPlan.bring.slice()
      ...
      while (seriesW<gamesNeeded && seriesL<gamesNeeded && gamesPlayed<bo) {
        const battle = simulateBattle(TEAMS[playerTeamKey], TEAMS[oppTeamKey], {
          format: opts.format || currentFormat,
          playerBring,
          opponentBring,
    ...
    if (onProgress) onProgress(i+bSize, effectiveSeriesCount, liveW, liveL);
  ...
  results.winRate = results.wins/effectiveSeriesCount;
  results.avgTurns = results.totalTurns/effectiveSeriesCount;
  results.avgTrTurns = results.totalTrTurns/effectiveSeriesCount;
  return results;
}
...
async function runAllMatchupsUI(numSeries, bo, onProgress, onDone) {
  const opps = getOrderedOpponentTeamKeys(currentPlayerKey, {
    includePlayer: false,
    ladderOnly: !!LADDER_MODE
  });
  const playerPlans = buildFormatAwareTeamPlans(currentPlayerKey, currentFormat);
  const effectiveSeriesCount = playerPlans.length ? Math.max(numSeries, playerPlans.length) : numSeries;
  try {
    ChampionsSim.state.lastRunAllOpponentKeys = opps.slice();
    ChampionsSim.state.lastRunAllPlayerPlans = playerPlans.slice();
  } catch (_e) {}
  let done=0;
  for (const opp of opps) {
    const res = await runBoSeries(numSeries,currentPlayerKey,opp,bo,(cur,tot,w,l)=>{
      if (onProgress) onProgress(done*effectiveSeriesCount+cur, opps.length*effectiveSeriesCount, w, l);
    }, {
      format: currentFormat,
...
function csStartFreshSimResultScope(teamKey, fmt) {
  if (typeof ChampionsSim === 'undefined') return;
  ChampionsSim.state.lastResults = {};
  ChampionsSim.state.lastResultsPlayerKey = teamKey || null;
  ChampionsSim.state.lastResultsFormat = fmt || (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles');
  delete ChampionsSim.state.lastTeamRunSnapshot;
  delete ChampionsSim.state.cachedStrategyOverride;
}

function csEnsureSimResultScope(teamKey, fmt) {
  if (typeof ChampionsSim === 'undefined') return;
  var format = fmt || (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles');
  if (!ChampionsSim.state.lastResults ||
      ChampionsSim.state.lastResultsPlayerKey !== teamKey ||
      ChampionsSim.state.lastResultsFormat !== format) {
    csStartFreshSimResultScope(teamKey, format);
  }
}

function csGetScopedStrategyResults(teamKey, fmt) {
  if (typeof ChampionsSim === 'undefined' || !ChampionsSim.state.lastResults) return {};
  var format = fmt || (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles');
  var owner = ChampionsSim.state.lastResultsPlayerKey || null;
  var ownerFormat = ChampionsSim.state.lastResultsFormat || null;
  if (owner && owner !== teamKey) return {};
  if (ownerFormat && ownerFormat !== format) return {};
  return ChampionsSim.state.lastResults || {};
}
...
document.getElementById('run-sim-btn')?.addEventListener('click', async function() {
  ...
  try {
    ...
    csEnsureSimResultScope(currentPlayerKey, currentFormat);
    ...
    try { csStoreTeamRunSnapshot(currentPlayerKey, oppKey, currentFormat, bo, 'single_sim'); } catch (_snapshotErr) {}
...
document.getElementById('run-all-btn')?.addEventListener('click', async function() {
  ...
  try {
    ...
    if (!TEAMS[currentPlayerKey]) throw new Error('player team not loaded: ' + currentPlayerKey);
    if (!Number.isFinite(n) || n < 1) throw new Error('invalid simulation count');
    csStartFreshSimResultScope(currentPlayerKey, currentFormat);
    try { csStoreTeamRunSnapshot(currentPlayerKey, '', currentFormat, bo, 'run_all'); } catch (_snapshotErr) {}
    const planCount = buildFormatAwareTeamPlans(currentPlayerKey, currentFormat).length;
    const effectiveSeriesCount = planCount ? Math.max(n, planCount) : n;
    const opponentCount = getOrderedOpponentTeamKeys(currentPlayerKey, {
      includePlayer: false,
      ladderOnly: !!LADDER_MODE
    }).length;
    document.getElementById('progress-wrap').style.display='';
    setProgress(0,'Starting… ' + opponentCount + ' opponents · ' + effectiveSeriesCount + ' series each',0,0);
    const matBadge=document.getElementById('matrix-badge');
    if(matBadge) matBadge.textContent=`${currentFormat==='doubles'?'Doubles':'Singles'} · Bo${bo} · ${opponentCount} opponents · ${effectiveSeriesCount} series each`;
...
function buildLeadSystem(results, playerMembers) {
  ...
  function fallbackPairFromRoles() {
    var members = Array.isArray(playerMembers) ? playerMembers.slice() : [];
    if (!members.length) return null;
    var classified = members.map(function(m){
      var roles = [];
      try {
        var c = (typeof classifyPokemon === 'function') ? classifyPokemon(m) : null;
        roles = c && Array.isArray(c.roles) ? c.roles.slice() : [];
      } catch (_e) {
        roles = [];
      }
      return { mon: m, roles: roles };
    });
    function hasRole(row, role) { return row.roles.indexOf(role) >= 0; }
    function hasAnyMove(row, moves) {
      return _pdfHasAny(row.mon, moves);
    }
    var support = classified.find(function(row){ return hasRole(row, 'Support') || hasAnyMove(row, PDF_FAKE_OUT) || hasAnyMove(row, PDF_REDIRECT); });
    var speed = classified.find(function(row){ return hasRole(row, 'Speed Control') || hasAnyMove(row, PDF_TAILWIND) || hasAnyMove(row, PDF_TRICK_ROOM); });
    var pivot = classified.find(function(row){ return hasRole(row, 'Pivot') || /U-turn|Volt Switch|Flip Turn|Parting Shot|Teleport/i.test((row.mon.moves || []).join(',')); });
    var breaker = classified.find(function(row){ return hasRole(row, 'Sweeper') || hasRole(row, 'Cleaner') || hasRole(row, 'Wallbreaker'); });
    var first = classified[0];
    var second = classified[1] || first;
    var pair = null;
    if (support && speed && support.mon.name !== speed.mon.name) pair = [support.mon.name, speed.mon.name];
    else if (support && breaker && support.mon.name !== breaker.mon.name) pair = [support.mon.name, breaker.mon.name];
    else if (speed && breaker && speed.mon.name !== breaker.mon.name) pair = [speed.mon.name, breaker.mon.name];
    else if (support && pivot && support.mon.name !== pivot.mon.name) pair = [support.mon.name, pivot.mon.name];
    else if (pivot && breaker && pivot.mon.name !== breaker.mon.name) pair = [pivot.mon.name, breaker.mon.name];
    else if (first && second && first.mon.name !== second.mon.name) pair = [first.mon.name, second.mon.name];
    if (!pair || !pair[0] || !pair[1]) return null;
    return pair.join(' + ');
  }
  if (!sawWinningLogs) {
    var fallbackSafe = fallbackPairFromRoles();
    if (fallbackSafe) {
      var fallbackSpeed = null;
      var fallbackPressure = null;
      var fallbackPunish = null;
      var members = Array.isArray(playerMembers) ? playerMembers.slice() : [];
      var supportMon = members.find(function(m){ return _pdfHasAny(m, PDF_FAKE_OUT) || _pdfHasAny(m, PDF_REDIRECT); }) || members[0] || null;
      var speedMon = members.find(function(m){ return _pdfHasAny(m, PDF_TAILWIND) || _pdfHasAny(m, PDF_TRICK_ROOM); }) || supportMon;
      var pressureMon = members.find(function(m){ return _pdfHasAny(m, PDF_PRIORITY) || _pdfHasAny(m, PDF_SPREAD); }) || speedMon;
      if (supportMon && speedMon) fallbackSpeed = [supportMon.name, speedMon.name].filter(Boolean).join(' + ');
      if (supportMon && pressureMon) fallbackPressure = [supportMon.name, pressureMon.name].filter(Boolean).join(' + ');
      if (pressureMon && speedMon) fallbackPunish = [pressureMon.name, speedMon.name].filter(Boolean).join(' + ');
      return {
        safe: fallbackSafe,
        speed: fallbackSpeed || fallbackSafe,
        pressure: fallbackPressure || fallbackSafe,
        punish: fallbackPunish || fallbackSafe
      };
    }
  }
  return {
...
function inferTeamIdentity(team, results, fmt) {
  ...
  var hasFakeOut = members.some(function(m){ return _pdfHasAny(m, PDF_FAKE_OUT); });
  var hasWeather = members.some(function(m){ return _pdfHasAny(m, PDF_WEATHER_ABILITIES) || (m.moves || []).some(function(move){
    return /Sunny Day|Rain Dance|Sandstorm|Snowscape|Hail/i.test(move);
  }); });
  ...
  function fallbackPrimary() {
    if (hasTR || playstyle.indexOf('Trick Room') >= 0) {
      return 'Set Trick Room, preserve the slow breakers, and clean with the team’s lowest-speed damage core.';
    }
    if (hasFakeOut && (hasTW || members.some(function(m){ return _pdfHasAny(m, PDF_PRIORITY); }))) {
      return 'Win turn 1 tempo with Fake Out plus speed control, then preserve the cleaner for the endgame.';
    }
    if (hasWeather || /Offense/i.test(playstyle)) {
      return 'Establish your weather or offensive mode early, keep the mode setter alive, and force the opponent to react.';
    }
    if (hasRedirect) {
      return 'Use redirection to create one clean setup window, then trade into your strongest finisher.';
    }
    return 'Use your support core to create a clean endgame for your best attacker.';
  }
  function fallbackSecondary() {
    if (hasTR) return 'Back up Trick Room with positioning and a preserved secondary breaker.';
    if (hasTW) return 'If speed control is denied, pivot to your pressure core and play for chip into a late clean.';
    if (hasWeather) return 'If the weather war is denied, win with positioning and chip rather than forcing a bad mirror.';
    return 'Keep a pivot piece alive so you can reset bad board states before the endgame.';
  }
  ...
  return {
    playstyle: playstyle,
    primary_win_condition: primary || fallbackPrimary(),
    secondary_win_condition: secondary || fallbackSecondary(),
...
function buildMatchupIntelligence(team, results, format, identity, leadSystem, trends, matchupGaps, matchupWarnings) {
  ...
  var confidence = totalGames > 0 ? csConfidence(totalGames) : 'none';
  var grade = totalGames === 0 ? 'unproven'
    : overallWR >= 0.65 ? 'strongly favored'
  ...
  return {
    ...
    overall_win_rate: overallWR,
      ...
      : (totalGames === 0
        ? 'You have no battle sample yet. Start from the baseline opener and collect games before judging matchups.'
    ...
    headline: topSafe.length
      ...
      : (totalGames === 0
        ? 'Top player read: use the baseline opener and test your mode before tuning the matchup map.'
    ...
    best_win_path: identity && identity.primary_win_condition ? identity.primary_win_condition : (totalGames === 0 ? 'baseline opener -> preserve the mode setter -> clean with your best attacker' : 'unclear'),
...
function buildWeaknessDashboard(team, results, format, identity, leadSystem, trends, deadMoves, matchupWarnings) {
  ...
  var totalGames = 0;
  Object.values(results || {}).forEach(function(r){
    totalGames += (r && r.wins ? r.wins : 0) + (r && r.losses ? r.losses : 0) + (r && r.draws ? r.draws : 0);
  });
  ...
  function _deadOwner(row) {
    return (row && (row.owner || row.pokemon || row.name)) || 'unknown';
  }
  function _deadTimes(row) {
    return (row && typeof row.times_used === 'number') ? row.times_used : ((row && typeof row.used === 'number') ? row.used : 0);
  }
  function _deadGames(row) {
    return (row && typeof row.games_sampled === 'number') ? row.games_sampled : ((row && typeof row.games === 'number') ? row.games : totalGames);
  }
  ...
  sections.push({
    ...
    headline: deadList.length
      ? (_deadOwner(deadList[0]) + ' - ' + deadList[0].move + ' has 0 calls in the sample.')
    ...
    rows: _itemRows(deadList, function(row){
      return {
        label: _deadOwner(row) + ' - ' + row.move,
        value: _deadTimes(row) + ' calls over ' + _deadGames(row) + ' games'
      };
    }),
...
function csRenderStrategyDataSources(report, teamKey, fromCache) {
  var compliance = report && report.team_compliance ? report.team_compliance : null;
  var complianceText = compliance ? (csInferComplianceLabel(compliance) + ' · ' + (compliance.ruleset_profile || 'unknown ruleset')) : 'Not checked';
  var simSample = report && report.sim_data_version ? report.sim_data_version : 0;
  var historyCount = 0;
  try {
    var h = (typeof computeTeamHistory === 'function') ? computeTeamHistory(teamKey) : null;
    historyCount = h && typeof h.total_battles === 'number' ? h.total_battles : 0;
  } catch (_e) {}
  var scoped = (typeof ChampionsSim !== 'undefined' && ChampionsSim.state.lastResultsPlayerKey)
    ? (ChampionsSim.state.lastResultsPlayerKey === teamKey ? 'Scoped to this active team' : 'Ignored because it belongs to another team')
    : 'Legacy/unscoped browser sample';
  var sourceMode = fromCache ? 'Cached Strategy snapshot' : (simSample > 0 ? 'Fresh simulation overlay' : 'Team theory + available local memory');
  var teamName = (typeof TEAMS !== 'undefined' && TEAMS[teamKey] && TEAMS[teamKey].name) ? TEAMS[teamKey].name : teamKey;
  return '' +
    '<section class="cs-section cs-data-sources">' +
      '<h3 class="cs-h3">Data Sources</h3>' +
      '<p class="cs-explain">This report is built from the data currently available for ' + _csEsc(teamName) + '. Free mode stays local and limited; paid mode can expand this into saved replay trends and long-term Battle IQ memory.</p>' +
      '<div class="cs-cat-grid">' +
        '<div class="cs-cat"><div class="cs-cat-label">Report source</div><div class="cs-cat-score">' + _csEsc(sourceMode) + '</div></div>' +
        '<div class="cs-cat"><div class="cs-cat-label">Current sim data</div><div class="cs-cat-score">' + _csEsc(String(simSample)) + ' games</div><small>' + _csEsc(scoped) + '</small></div>' +
        '<div class="cs-cat"><div class="cs-cat-label">Local trend memory</div><div class="cs-cat-score">' + _csEsc(String(historyCount)) + ' games</div><small>Browser-only until profile sync exists.</small></div>' +
        '<div class="cs-cat"><div class="cs-cat-label">Compliance source</div><div class="cs-cat-score">' + _csEsc(complianceText) + '</div></div>' +
      '</div>' +
      '<div class="cs-strategy-actions">' +
        '<button class="secondary-btn" type="button" id="strategy-clear-data-btn">Clear Strategy data</button>' +
  ...
  return {
    ...
    cards: [
      ...
      },
      {
        ...
        detail: replayTeamMatch ? (replayComparisonMode + ' · ' + (replayTeamMatch.status || 'unknown') + ' · ' + (replayTeamMatch.summary || '')) : replayComparisonMode,
...
function renderSourcesTab(teamKey) {
  ...
  html += '<section class="cs-section cs-data-sources">' +
    ...
    '<div class="replay-coach-summary-grid">' +
      ...
      '<div class="replay-coach-metric"><strong>Session team snapshot</strong><span>' + _escapeHtml(model.teamRunSnapshot ? (model.teamRunSnapshot.teamFingerprint.displayName + ' · ' + model.teamRunSnapshot.format) : 'none yet') + '</span></div>' +
      '<div class="replay-coach-metric"><strong>Replay team match</strong><span>' + _escapeHtml(model.replayTeamMatch ? (model.replayTeamMatch.status || 'unknown') : 'none yet') + '</span></div>' +
    '</div>' +
  '</section>';
...
function renderStrategyTab(teamKey) {
  ...
  if (ChampionsSim.state.cachedStrategyOverride) {
    ...
    var strategyFormat = (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles');
    var results = (typeof csGetScopedStrategyResults === 'function') ? csGetScopedStrategyResults(teamKey, strategyFormat) : ((typeof window !== 'undefined' && ChampionsSim.state.lastResults) ? ChampionsSim.state.lastResults : {});
    report = csBuildStrategyReportV2(teamKey, results, strategyFormat);
  }
  if (!report) {
  ...
  html += csRenderStrategyDataSources(report, teamKey, fromCache);
  ...
  var strategyClearBtn = host.querySelector ? host.querySelector('#strategy-clear-data-btn') : null;
  if (strategyClearBtn) {
    strategyClearBtn.addEventListener('click', function(){
      try {
        if (typeof csClearStrategyData === 'function') csClearStrategyData(teamKey, { render: true });
      } catch (e) { UILog.warn('strategy clear failed', e); }
    });
  }
...
function csClearLegacyStrategyReport(teamKey) {
  var team = (typeof TEAMS !== 'undefined' && TEAMS[teamKey]) ? TEAMS[teamKey] : null;
  if (!team || typeof Storage === 'undefined' || typeof Storage.del !== 'function') return false;
  try {
    Storage.del(T9J16_STORAGE_KEY + '::' + teamSignature(team));
    return true;
  } catch (e) {
    UILog.warn('legacy strategy clear failed', e);
    return false;
  }
}

function csClearStrategyData(teamKey, opts) {
  var key = teamKey || (typeof currentPlayerKey !== 'undefined' ? currentPlayerKey : null);
  if (!key || typeof TEAMS === 'undefined' || !TEAMS[key]) return { ok: false, reason: 'unknown_team' };
  var cleared = {
    ok: true,
    report: false,
    legacyReport: false,
    simLog: false,
    memoryCache: false,
    transientResults: false
  };
  try { cleared.report = !!csClearReport(key); } catch (_reportErr) {}
  try { cleared.legacyReport = !!csClearLegacyStrategyReport(key); } catch (_legacyErr) {}
  try { if (typeof csSimLogClearTeam === 'function') cleared.simLog = !!csSimLogClearTeam(key); } catch (_simErr) {}
  try { if (typeof csInvalidateTeamHistory === 'function') csInvalidateTeamHistory(key); } catch (_histErr) {}
  try { if (typeof csClearStrategyReportCache === 'function') { csClearStrategyReportCache(); cleared.memoryCache = true; } } catch (_cacheErr) {}
  try {
    if (typeof ChampionsSim !== 'undefined') {
      var owner = ChampionsSim.state.lastResultsPlayerKey || null;
      if (!owner || owner === key) {
        ChampionsSim.state.lastResults = {};
        ChampionsSim.state.lastResultsPlayerKey = key;
        ChampionsSim.state.lastResultsFormat = (typeof currentFormat !== 'undefined' ? currentFormat : 'doubles');
        delete ChampionsSim.state.lastTeamRunSnapshot;
        cleared.transientResults = true;
      }
      delete ChampionsSim.state.lastStrategyReport;
      delete ChampionsSim.state.cachedStrategyOverride;
    }
  } catch (_stateErr) {}
  if (!opts || opts.render !== false) {
    try { if (typeof renderStrategyTab === 'function') renderStrategyTab(key); } catch (_renderErr) {}
  }
  return cleared;
}
...
function csSimLogClearTeam(teamKey) {
  var store = _csSimLogRead();
  store.entries = store.entries.filter(function(e){ return e.playerKey !== teamKey && e.oppKey !== teamKey; });
  var ok = _csSimLogWrite(store);
  try { if (typeof csInvalidateTeamHistory === 'function') csInvalidateTeamHistory(teamKey); } catch (_e) {}
  return ok;
}
...
if (typeof ChampionsSim !== 'undefined') {
  ...
  ChampionsSim.strategy.csClearStrategyData = csClearStrategyData;
}
...
if (typeof exposeLegacyWindowAlias === 'function') exposeLegacyWindowAlias('csClearStrategyData', csClearStrategyData);
