...
T('5. Overview styles are responsive and scan-friendly', () => {
  ...
  inc(css, '.home-action-buttons');
  inc(css, '.home-cycle-stage');
  inc(css, '@keyframes homeCycleSpin');
...
T('6. Overview renders through a reusable function for future growth', () => {
  ...
  inc(ui, 'One loop: test, learn, improve.');
  inc(ui, 'Pick one team question. Run one test or upload one replay. Change one thing. Run it again.');
  inc(ui, 'Run a battle');
  inc(ui, 'Upload replay');
  inc(ui, 'Edit team');
  inc(ui, 'See Top 25');
  ...
  inc(ui, 'Test > Learn > Improve');
  inc(ui, 'home-cycle-node-a');
