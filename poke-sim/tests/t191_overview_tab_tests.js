...
T('5. Overview styles are responsive and scan-friendly', () => {
  ...
  inc(css, '.home-value-strip article strong{font-size:clamp(22px,2.35vw,34px)');
...
T('6. Overview renders through a reusable function for future growth', () => {
  ...
  inc(ui, 'Test the team');
  inc(ui, 'Learn the turn');
  inc(ui, 'Try one fix');
  ...
  inc(ui, 'Easy enough to start in one minute.');
  inc(ui, 'Pick a team');
  inc(ui, 'Run a battle test');
  inc(ui, 'Read what happened');
  inc(ui, 'Try one change');
