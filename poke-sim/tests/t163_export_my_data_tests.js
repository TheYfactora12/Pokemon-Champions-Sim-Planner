...
async function main() {
  ...
  await T('6. QA artifact includes retention caps, build, source, and retained evidence', async () => {
    ...
    truthy(/^v2\.1\.\d+-[a-z0-9-]+$/.test(payload.build_id || ''), 'QA build id missing');
    truthy(/^http:\/\/localhost\/\?v=v2\.1\.\d+-[a-z0-9-]+&fresh=1$/.test(payload.source_url || ''), 'QA source URL missing build cache buster');
