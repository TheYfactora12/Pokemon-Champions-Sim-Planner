...
Implementation start: `v2.2.39-app-shell-security-split` adds `release_manifest.js` as the first source-of-truth layer for visible build ID, export `build_id`, service worker cache identity, source-sync policy, committed bundle SHA, and deployed GitHub Pages artifact SHA. `v2.2.39-app-shell-security-split` starts the public launch security gate by testing that local credentials stay ignored, committed app shells keep Supabase placeholders empty, Pages deploy injects only anon/public runtime config, and service-role/write/database secrets cannot land in browser runtime files. `v2.2.39-app-shell-security-split` adds a baseline Content Security Policy plus CI checks for dynamic code execution, inline event-handler attributes, Pages CSP preservation, and existing public XSS regression coverage. Because the current static bundle still needs inline script/style allowances, removing those allowances remains part of the UI/runtime split milestone rather than a hidden claim of full CSP lockdown. `v2.2.39-app-shell-security-split` also starts the UI/runtime split by moving release identity, runtime error surfacing, build-cache reload, source URL proof, sprite fallback handling, and delegated public-security handlers into `app_shell.js` so launch/security boot code has a smaller audited owner surface. Login and saved player profiles remain future-gated: accounts should only persist teams, sim history, replay summaries, cross-device progress, and player-pattern learning after Supabase Auth, RLS ownership checks, per-user data separation, export/delete controls, and consent-safe aggregation are implemented.
