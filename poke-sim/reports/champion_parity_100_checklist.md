...
- Current local build target: `v2.1.35-low-kick-weight-parity`
...
- Showdown damage oracle: `54/54` local cases passing after the Low Kick slice.
- Move support audit: `85 verified`, `35 baseline`, `0 incomplete` across `120` shipped distinct moves.
...
- Fresh live proof still needed after this release: GitHub Pages `?v=<new-sha>`, one single-run log, one Run All log, and one QA Artifact from `v2.1.35`.

## Do Not Claim Broad 100% Until

- Shipped moves have `0 incomplete` and every high-impact baseline move has explicit regression or oracle coverage.
...
## Remaining Baseline Move Groups
...
4. Simple damage-only coverage: Aura Sphere, Dragon Pulse, Hydro Pump, Liquidation, Power Gem, Psychic, Throat Chop, Kowtow Cleave, Darkest Lariat, Leaf Storm, Light of Ruin, Dual Wingbeat.
5. Any move promoted from baseline must name the source truth, test file, and exact behavior covered in `move_support.js`.
