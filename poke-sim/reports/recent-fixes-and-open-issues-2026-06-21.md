...
This snapshot records the current truth after the June 21-23 live-log review, GitHub issue sweep, Alfredo sync, and Low Kick weight-parity pass. It is meant to remove stale contradictions before the next mechanics-parity pass.

## 2026-06-23 Deployment Update

- The prior Y fork public validation target was commit `7e0deca` (`model champion ability parity`).
- `v2.1.35-low-kick-weight-parity` supersedes `v2.1.34-live-log-proof` for the next GitHub Pages release and bumps the service-worker cache to `champions-sim-v69-low-kick-weight-parity`.
...
- Local move support now reports `85 verified`, `35 baseline`, `0 incomplete`; the Showdown damage oracle is `54/54` after Low Kick target-weight coverage.
...
| 100% Champion parity gate | Documented in `v2.1.35-low-kick-weight-parity` | `reports/champion_parity_100_checklist.md` defines the practical gate: verified high-impact moves, legal Champion teams, DB override guards, browser single/Run All/QA proof, source-version agreement, and visible gaps before accuracy claims. |
| Move support audit | Improved in `v2.1.35-low-kick-weight-parity` | `reports/move_support_audit.md` now reports `85 verified`, `35 baseline`, `0 incomplete` across `120` shipped distinct moves. |
...
| GitHub Pages cache drift | Guarded for this release | `sw.js` cache bumped to `champions-sim-v69-low-kick-weight-parity`; `index.html`, `ui.js`, and bundled `pokemon-champion-2026.html` carry `v2.1.35-low-kick-weight-parity` after bundle rebuild. |
...
| Overview truth board | Updated in `v2.1.35-low-kick-weight-parity` | The live Overview tab now names the target bridge fix, DB/runtime source split, capped browser log retention, shipped QA artifact export, type audit, Low Kick weight parity, typed held-item boost fix, Knock Off behavior fix, Tera Blast parity, no-valid-target guard, Champion SP spread guard, editor-builder UX gap, latest live-log proof, stat/speed/export evidence, 100% checklist, remaining grouped mechanics gap, and completed Alfredo sync. |
...
- Fixed for `v2.1.35`: Low Kick now uses target Showdown species weight instead of producing zero damage from Showdown `basePower: 0`; the oracle is 54/54 and the move audit is 85 verified / 35 baseline / 0 incomplete.
...
- Not fixed yet: full move/damage/regional-form parity audit and remaining grouped battle mechanics.
- Not ready for broad accuracy claims: sim still needs grouped mechanics parity work and Showdown/Champions oracle gates.

## Current Next Path

1. Export one fresh single-run log, one fresh Run All log, and one `QA Artifact` from the public URL after `v2.1.35-low-kick-weight-parity`; verify build/source metadata, stable turn-log fields, legal Champion SP team data, `speed_order_details`, `stat_boosts`, `damage_events`, Low Kick damage evidence when present, Tera Blast evidence when present, `knock_off_boost` evidence when Knock Off appears, no team-load failure, no live-target `(no valid target)` failures, and retained-evidence summary counts.
2. Mirror/update issue notes in the Y fork for Alfredo #241, #240, and #231 so both repos show the same truth.
3. Continue converting the 35 remaining baseline moves into verified mechanics coverage, starting with spread/weather accuracy, variable base-power and item-state moves, secondary status/stat effects, damage modifiers, targeting/redirection, protection, switching, and status.
