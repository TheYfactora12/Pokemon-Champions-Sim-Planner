...
- `v2.1.41-recoil-showdown-context` supersedes `v2.1.40-turn-log-team-roster` for the next GitHub Pages release and bumps the service-worker cache to `champions-sim-v75-recoil-showdown-context`.
- `v2.1.41` preserves Pokemon Showdown `data/text/moves.ts` descriptions and move `recoil` tuples in generated move metadata. The Overview-linked QA Baseline Snapshot now exposes recoil rules and Showdown context for shipped recoil moves, and focused recoil tests assert exact applied-damage ratios for Flare Blitz, Wave Crash, and Head Smash.
...
- Current local truth after the damage-log/catalog, export-visibility, and recoil-context pass: the confirmed applied-vs-calculated damage logging bug is fixed, recoil is based on applied target HP loss, the runtime catalog has a top-10 approved Champion archetype set, and one single-run plus seventeen Run All/browser exports validate structurally after two validator-only false positives were fixed. The sim is not ready for broad 100% accuracy claims until QA Artifact proof, fresh v2.1.41 deployed-browser proof, source-drift guardrails, and the remaining grouped mechanics/stress gates are complete.
...
- Alfredo PR #245 merged the prior Champion parity tree after required checks passed; TheYfactora12 main was fast-forwarded to the same merge commit. The current v2.1.41 recoil-showdown-context slice should prove out on the Y fork first, with Alfredo sync handled later by PR notes.
...
| GitHub Pages cache drift | Guarded for this release | `sw.js` cache bumped to `champions-sim-v75-recoil-showdown-context`; `index.html`, `ui.js`, and bundled `pokemon-champion-2026.html` must carry `v2.1.41-recoil-showdown-context` after bundle rebuild. |
...
| Overview truth board | Updated in `v2.1.41-recoil-showdown-context` | The live Overview tab now names the target bridge fix, DB/runtime source split, capped browser log retention, shipped QA artifact export, type audit, Low Kick weight parity, core shipped move parity, typed held-item boost fix, Knock Off behavior fix, Tera Blast parity, no-valid-target guard with replacement-timing correction, stable action identity export, full turn-log team metadata, QA baseline snapshot, Showdown recoil context, Champion SP spread guard, editor-builder UX gap, latest live-log proof, stat/speed/export evidence, 100% checklist, approved top-10 catalog, current GitHub sim-truth trackers, remaining non-move truth gates, and prior Alfredo sync status. |
| QA baseline snapshot | Updated in `v2.1.41-recoil-showdown-context` | `reports/champion_qa_baseline_snapshot.md` is generated from `data.js`, generated Showdown legal data, Showdown `data/text/moves.ts`, and `move_support.js`. It lists approved Champion runtime team movesets, approved-catalog move metadata, recoil rules, Showdown move context, support status, tests, notes, and all shipped move support. `qa_baseline_snapshot_tests.js` keeps the report linked from Overview and fails if approved team/move rows drift without regeneration. |
...
| Alfredo | [#239](https://github.com/alfredocox/Pokemon-Champions-Sim-Planner/issues/239) | Companion tracker for reviewing Showdown DB sync, runtime path, and static fallback behavior. |
...
| Y fork | [#137](https://github.com/TheYfactora12/Pokemon-Champions-Sim-Planner/issues/137) | Tracks Showdown DB sync/runtime/fallback audit. Needs update or sibling links for Alfredo #241/#239 so the fork does not look more complete than upstream. |
...
- Tracked for next reliability pass: `POKESIM_CODEX_STRESS_TEST_BRIEF.md` defines a broader deterministic stress, invariant, replay fuzz, persistence-fault, browser-smoke, bundle-drift, and CI automation program, and maps to Alfredo #206/#204 plus Y #96/#94. This is not complete in v2.1.41; this slice only implements the relevant stable action identity, QA data visibility, and Showdown recoil context contracts.
...
- Previously completed: Alfredo PR #245 merged and both `alfredocox/main` and `TheYfactora12/main` pointed to the same source tree before the current v2.1.41 local slice.
...
- Not fixed yet: live DB `showdown_entities` as the approved runtime source, regional-form review closure, full six-mon replay parsing, deployment/cache hardening beyond local cache bumps, and remaining grouped battle-system mechanics beyond the shipped move audit.
- Not ready for broad accuracy claims: sim still needs QA Artifact proof, fresh deployed-browser v2.1.41 proof after this guardrail/bundle update, source-drift guardrails, the stress-automation program, and Showdown/Champions oracle gates for future data changes.

## Current Next Path

1. Export one fresh single-run log, one fresh Run All log, and one `QA Artifact` from the public URL after `v2.1.41-recoil-showdown-context`; verify build/source metadata, full `player_team`/`opponent_team` metadata, stable turn-log fields, action `actor_key`/`target_key` identity fields, legal Champion SP team data, `speed_order_details`, `stat_boosts`, `damage_events`, recoil evidence when present, Low Kick/Tera Blast/Knock Off evidence when present, secondary-effect evidence when present, no team-load failure, no true live-target `(no valid target)` failures, and retained-evidence summary counts.
2. Run the long-run/golden trace audit after v2.1.41 because intentional catalog/logging/validator/export fixes can change deterministic traces or exported proof even when winners remain valid.
...
9. Update Y #137/#123 with the active Alfredo source-truth tickets (#241/#239/#231) and the closed state of Alfredo #240 so both repos tell the same story.
