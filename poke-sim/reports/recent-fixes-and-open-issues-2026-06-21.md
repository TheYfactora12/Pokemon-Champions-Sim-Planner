...
- `v2.1.38-log-validator-replacement` supersedes `v2.1.37-damage-log-team-catalog` for the next GitHub Pages release and bumps the service-worker cache to `champions-sim-v72-log-validator-replacement`.
...
- Current local truth after the damage-log/catalog and validator-replacement pass: the confirmed applied-vs-calculated damage logging bug is fixed, the runtime catalog has a top-10 approved Champion archetype set, and one single-run plus four Run All doubles browser exports validate structurally. The sim is not ready for broad 100% accuracy claims until QA Artifact proof, fresh v2.1.38 deployed-browser proof, source-drift guardrails, and the remaining grouped mechanics gates are complete.
...
- Alfredo PR #245 merged the prior Champion parity tree after required checks passed; TheYfactora12 main was fast-forwarded to the same merge commit. The current v2.1.38 validator-replacement slice should prove out on the Y fork first, with Alfredo sync handled later by PR notes.
...
| Target-safe log validation | Guarded through `v2.1.38-log-validator-replacement` | `validate-turn-logs.mjs` rejects `(no valid target)` skips when an original target-side active Pokemon remains live, while preserving legitimate terminal skips after an earlier KO empties that side. `v2.1.38` fixes a false positive where post-turn replacements were counted as available before their send-out event. |
...
| GitHub Pages cache drift | Guarded for this release | `sw.js` cache bumped to `champions-sim-v72-log-validator-replacement`; `index.html`, `ui.js`, and bundled `pokemon-champion-2026.html` must carry `v2.1.38-log-validator-replacement` after bundle rebuild. |
...
| Overview truth board | Updated in `v2.1.38-log-validator-replacement` | The live Overview tab now names the target bridge fix, DB/runtime source split, capped browser log retention, shipped QA artifact export, type audit, Low Kick weight parity, core shipped move parity, typed held-item boost fix, Knock Off behavior fix, Tera Blast parity, no-valid-target guard with replacement-timing correction, Champion SP spread guard, editor-builder UX gap, latest live-log proof, stat/speed/export evidence, 100% checklist, approved top-10 catalog, remaining non-move truth gates, and prior Alfredo sync status. |
...
- Previously completed: Alfredo PR #245 merged and both `alfredocox/main` and `TheYfactora12/main` pointed to the same source tree before the current v2.1.38 local slice.
...
- Not ready for broad accuracy claims: sim still needs QA Artifact proof, fresh deployed-browser v2.1.38 proof after this guardrail/bundle update, source-drift guardrails, and Showdown/Champions oracle gates for future data changes.

## Current Next Path

1. Export one fresh single-run log, one fresh Run All log, and one `QA Artifact` from the public URL after `v2.1.38-log-validator-replacement`; verify build/source metadata, stable turn-log fields, legal Champion SP team data, `speed_order_details`, `stat_boosts`, `damage_events`, Low Kick/Tera Blast/Knock Off evidence when present, secondary-effect evidence when present, no team-load failure, no true live-target `(no valid target)` failures, and retained-evidence summary counts.
2. Run the long-run/golden trace audit after v2.1.38 because intentional catalog/logging/validator fixes can change deterministic traces or exported proof even when winners remain valid.
