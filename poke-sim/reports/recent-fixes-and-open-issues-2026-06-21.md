...
- `v2.1.37-damage-log-team-catalog` supersedes `v2.1.36-core-move-parity` for the next GitHub Pages release and bumps the service-worker cache to `champions-sim-v71-damage-log-team-catalog`.
...
- Current local truth after the damage-log/catalog pass: the confirmed applied-vs-calculated damage logging bug is fixed locally and the runtime catalog now has a top-10 approved Champion archetype set, but the sim is not ready for broad 100% accuracy claims until full stress, bundle rebuild, deployed browser logs, and QA exports are complete.
...
- Alfredo PR #245 merged the prior Champion parity tree after required checks passed; TheYfactora12 main was fast-forwarded to the same merge commit. The current v2.1.37 damage-log/team-catalog slice should prove out on the Y fork first, with Alfredo sync handled later by PR notes.
...
| GitHub Pages cache drift | Guarded for this release | `sw.js` cache bumped to `champions-sim-v71-damage-log-team-catalog`; `index.html`, `ui.js`, and bundled `pokemon-champion-2026.html` must carry `v2.1.37-damage-log-team-catalog` after bundle rebuild. |
...
| Overview truth board | Updated in `v2.1.37-damage-log-team-catalog` | The live Overview tab now names the target bridge fix, DB/runtime source split, capped browser log retention, shipped QA artifact export, type audit, Low Kick weight parity, core shipped move parity, typed held-item boost fix, Knock Off behavior fix, Tera Blast parity, no-valid-target guard, Champion SP spread guard, editor-builder UX gap, latest live-log proof, stat/speed/export evidence, 100% checklist, approved top-10 catalog, remaining non-move truth gates, and prior Alfredo sync status. |
...
- Previously completed: Alfredo PR #245 merged and both `alfredocox/main` and `TheYfactora12/main` pointed to the same source tree before the current v2.1.37 local slice.
...
- Not ready for broad accuracy claims: sim still needs deployed-browser v2.1.37 proof, source-drift guardrails, and Showdown/Champions oracle gates for future data changes.

## Current Next Path

1. Export one fresh single-run log, one fresh Run All log, and one `QA Artifact` from the public URL after `v2.1.37-damage-log-team-catalog`; verify build/source metadata, stable turn-log fields, legal Champion SP team data, `speed_order_details`, `stat_boosts`, `damage_events`, Low Kick/Tera Blast/Knock Off evidence when present, secondary-effect evidence when present, no team-load failure, no live-target `(no valid target)` failures, and retained-evidence summary counts.
2. Run the long-run/golden trace audit after v2.1.37 because intentional catalog/logging fixes can change deterministic traces or exported proof even when winners remain valid.
