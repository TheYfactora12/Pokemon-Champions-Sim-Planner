...
- `v2.1.39-stable-action-identity` supersedes `v2.1.38-log-validator-replacement` for the next GitHub Pages release and bumps the service-worker cache to `champions-sim-v73-stable-action-identity`.
...
- Current local truth after the damage-log/catalog and stable-action-identity pass: the confirmed applied-vs-calculated damage logging bug is fixed, the runtime catalog has a top-10 approved Champion archetype set, and one single-run plus eleven Run All/browser exports validate structurally after two validator-only false positives were fixed. The sim is not ready for broad 100% accuracy claims until QA Artifact proof, fresh v2.1.39 deployed-browser proof, source-drift guardrails, and the remaining grouped mechanics/stress gates are complete.
...
- Alfredo PR #245 merged the prior Champion parity tree after required checks passed; TheYfactora12 main was fast-forwarded to the same merge commit. The current v2.1.39 stable-action-identity slice should prove out on the Y fork first, with Alfredo sync handled later by PR notes.
...
| GitHub Pages cache drift | Guarded for this release | `sw.js` cache bumped to `champions-sim-v73-stable-action-identity`; `index.html`, `ui.js`, and bundled `pokemon-champion-2026.html` must carry `v2.1.39-stable-action-identity` after bundle rebuild. |
...
| Overview truth board | Updated in `v2.1.39-stable-action-identity` | The live Overview tab now names the target bridge fix, DB/runtime source split, capped browser log retention, shipped QA artifact export, type audit, Low Kick weight parity, core shipped move parity, typed held-item boost fix, Knock Off behavior fix, Tera Blast parity, no-valid-target guard with replacement-timing correction, stable action identity export, Champion SP spread guard, editor-builder UX gap, latest live-log proof, stat/speed/export evidence, 100% checklist, approved top-10 catalog, current GitHub sim-truth trackers, remaining non-move truth gates, and prior Alfredo sync status. |
...
| Y fork | [#105](https://github.com/TheYfactora12/Pokemon-Champions-Sim-Planner/issues/105) / [#104](https://github.com/TheYfactora12/Pokemon-Champions-Sim-Planner/issues/104) / Alfredo [#216](https://github.com/alfredocox/Pokemon-Champions-Sim-Planner/issues/216) | Manual Champion replay smoke and second verified replay artifact remain open; v2.1.39 fresh single, Run All, and QA Artifact exports should feed this proof. |
...
- Tracked for next reliability pass: `POKESIM_CODEX_STRESS_TEST_BRIEF.md` defines a broader deterministic stress, invariant, replay fuzz, persistence-fault, browser-smoke, bundle-drift, and CI automation program, and maps to Alfredo #206/#204 plus Y #96/#94. This is not complete in v2.1.39; this slice only implements the relevant stable action identity contract and regression coverage.
...
- Previously completed: Alfredo PR #245 merged and both `alfredocox/main` and `TheYfactora12/main` pointed to the same source tree before the current v2.1.39 local slice.
...
- Not ready for broad accuracy claims: sim still needs QA Artifact proof, fresh deployed-browser v2.1.39 proof after this guardrail/bundle update, source-drift guardrails, the stress-automation program, and Showdown/Champions oracle gates for future data changes.

## Current Next Path

1. Export one fresh single-run log, one fresh Run All log, and one `QA Artifact` from the public URL after `v2.1.39-stable-action-identity`; verify build/source metadata, stable turn-log fields, action `actor_key`/`target_key` identity fields, legal Champion SP team data, `speed_order_details`, `stat_boosts`, `damage_events`, Low Kick/Tera Blast/Knock Off evidence when present, secondary-effect evidence when present, no team-load failure, no true live-target `(no valid target)` failures, and retained-evidence summary counts.
2. Run the long-run/golden trace audit after v2.1.39 because intentional catalog/logging/validator/export fixes can change deterministic traces or exported proof even when winners remain valid.
