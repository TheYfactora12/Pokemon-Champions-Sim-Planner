...
const url = 'https://web-view.app.pokemonchampions.jp/battle/pages/events/rs178066986988lmoqpm/en/pokemon.html';
...
const artifact = {
  ...
  source_url: url,
  parent_notice_url: 'https://champions-news.pokemon-home.com/en/page/776.html',
  ...
  rows: rows.map(([official_id, eligible, label]) => ({ official_id, eligible: eligible === 1, label }))
};
fs.writeFileSync(new URL('../source/reg-m-b-official-roster.json', import.meta.url), JSON.stringify(artifact, null, 2) + '\n');
console.log(`Captured ${rows.length} official rows; ${rows.filter(row => row[1] === 1).length} eligible. No runtime promotion.`);
