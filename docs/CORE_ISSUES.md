...
- Damage formula must match the canonical Gen 9 order: `floor( floor( floor(2xL/5+2) x BP x A/D ) / 50 + 2 )`
- Nature, item, STAB, type effectiveness, spread modifier (0.75x), and burn penalty must all apply in the correct sequence
...
- Tailwind (x2 Speed) and other Speed modifiers must stack correctly
...
- **Abilities:** Intimidate on switch-in, Protean once per entry, Parental Bond child hit at 1/4 power, Unseen Fist 25% through Protect

---

## 5. Conditions & Statuses
- Burn (-1/2 physical damage, 1/16 chip), Paralysis (-50% Speed, 12.5% full-para), Poison (1/8 chip), Toxic (escalating N/16 capped at 15)
...
Data source: **Pokemon Showdown live data CDN** - https://play.pokemonshowdown.com/data/
