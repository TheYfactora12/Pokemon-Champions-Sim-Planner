...
- Official promotion still needs trusted worker, mapping, stale rules, privacy filtering, and sample-size gates.

Next useful work:

- Implement #188 mapping resolver first because every promotion rule depends on knowing which durable Team Lab team an artifact refers to.
- Then implement #189 promotion rules.
- Then connect import worker/pipeline.
...
1. Team Lab artifact team-ID mapping resolver (#188).
2. Team Lab promotion rules (#189) after mapping resolver.
